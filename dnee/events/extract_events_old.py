...
def find_arguments(key_dep, deps, target_deps):
...
def get_arguments(key_dep, xcomp, deps, entities, toks, sent_num, target_deps):
    arg_deps = find_arguments(key_dep, deps, target_deps)
    tmp_arg_texts = []
    tmp_args = []
    tmp_anis = []
    tmp_deps = []
...
def get_all_entities(doc_corefs):
    entities = {}
    for coref_key, corefs in doc_corefs.iteritems():
        # for each entiy
    ...
    for key_dep in key_deps:
        ...
        if do_create_event:
            ...
            tmp_arg1, tmp_ani1, tmp_dep1, tmp_arg1_text = \
                    get_arguments(key_dep, xcomp,
                                  deps, entities,
                                  toks, sent_num, ['dobj', 'iobj', 'nsubjpass'])
            if len(tmp_arg1) > 0:
